...
class KernelUserApplicationsModel extends KernelModelAbstract implements KernelModelInterface {
	
	use KernelModelTrait;
	
	private $id;
	private $table = TT_TABLE_PREFIX."user_applications";
	
	public function getUserApplications($uid) {
		$d = $this->d($this->table);
		$result = $d->get("id, uid, aid", "where uid=$uid", "order by id");
		if ($result) {
			return $result;
		} else {
			return false;
