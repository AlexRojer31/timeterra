...
class KernelApplicationTagsModel extends KernelModelAbstract implements KernelModelInterface {
	
	use KernelModelTrait;
	
	private $id;
	private $table = TT_TABLE_PREFIX."application_tags";
	...
	public function getTagInfo() {
		$d = $this->d($this->table);
		$id = $this->id;
		$result = $d->get("id, name, description, icon", "where id=$id", "order by id", "desc", "limit 1");
		if ($result) {
			return $result[0];
		} else {
			return false;
