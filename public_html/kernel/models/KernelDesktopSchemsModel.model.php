...
class KernelDesktopSchemsModel extends KernelModelAbstract implements KernelModelInterface {
	
	use KernelModelTrait;
	
	private $id;
	private $table = TT_TABLE_PREFIX."desktop_schems";
	
	public function getSchema($name) {
		$d = $this->d($this->table);
		$id = $this->id;
		$result = $d->get("id, name, parameter, value", "where name='$name'", "order by id");
		if ($result) {
			return $result;
		} else {
			return false;
