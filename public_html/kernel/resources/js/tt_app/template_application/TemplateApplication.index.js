...
function TemplateApplication() {
	...
	this.main = function() {
		let appId = 'TemplateApplication';
		let appWidth = 300;
		let appHeight = 300;
		let appName = 'Tempate application';
		let appIcon = '/kernel/resources/media/tt_application_icons/logo.svg';
		let additionalScripts = [
			'/kernel/resources/js/tt_app/template_application/TemplateAdditionalScript.js'
		];
		this.create(appId, appWidth, appHeight, appName, appIcon, additionalScripts);
	}
	
	this.construct = function(workField) {
		let test = new TemplateAdditionalScript();
		workField.innerHTML = test.sayHello();
	}
